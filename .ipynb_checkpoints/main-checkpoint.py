...
from models.t5 import T5PromptForConditionalGeneration_param
from utils.data_utils import AutoTask
from utils.eval_utils import AutoPostProcessor
...
from method import soft_prompt_dict
from cfg import CFG as cfg_m
...
###################################################################################################
    ...
    prompter = soft_prompt_dict[config.method](cfg=cfg_m,
                                               prefix_len=args.enc_prompt_tokens, 
                                               emb_size=emb_size,
                                               )
    
    model = T5PromptForConditionalGeneration_param.from_pretrained(args.model_name, config=config, prompter=prompter)

    # Freeze the backbone model
    for name, param in model.named_parameters():
        param.requires_grad = False if 'prompter' not in name else True
    ...
    if is_master:
        print('Parameters to optimize: ', [n for n, p in model.named_parameters() if 'prompter' in n])
    optimizer_grouped_parameters = [
        {
            "params": [p for n, p in model.named_parameters() if 'prompter' in n],
    ...
    if is_master:
        print('Prefix Parameters: ', [n for n, p in model.named_parameters() if 'prompter' in n])
    ...
    while True:
                    ...
                    if is_master and args.save_ckpt_dir:  # save the prompts
                        if global_step % args.saving_steps == 0:
                            checkpoint = {
                                ...
                                'params': {n: p for n, p in model.named_parameters() if 'prompter' in n},
        ...
        if is_master:
            ...
            save_txt_path = os.path.join(save_path, 'result.csv')
            
            # with open(save_txt_path, "a+") as file:
            #     file.write(f"{best_acc} ")
            mode = 'a' if os.path.exists(save_txt_path) else 'w'
            with open(save_txt_path, mode=mode, newline='') as file:
                writer = csv.writer(file)

                if mode == 'w':
                    writer.writerow(['trainable_params','accuracy','memo'])
        
                # 정확도 리스트의 각 항목을 CSV 파일에 추가
                writer.writerow([sum(p.numel() for p in model.parameters() if p.requires_grad), best_epoch_dev, args.memo])
