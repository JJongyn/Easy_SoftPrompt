...
from .metrics import *
from .util import round_stsb_target, pad_punctuation
...
class MRPC(AbstractTask):
    name = "mrpc"
    labels_list = ["0", "1"]
    metric = [accuracy, f1_score_with_invalid]
...
class STSB(AbstractTask):
    ...
    metric = [pearson_corrcoef, spearman_corrcoef]
...
class SuperGLUECB(AbstractTask):
    ...
    split_to_data_split = {"train": "train",
                           "validation": "validation",
                           "test": "validation"}
    metric = [mean_multiclass_f1(num_classes=3), accuracy]
...
class SuperGLUEMultiRC(AbstractTask):
    ...
    split_to_data_split = {"train": "train",
                           "validation": "validation",
                           "test": "validation"}
    metric = [multirc_f1_over_all_answers,
              mean_group_metric(exact_match)]
    metric_names = ["f1", "em"]
